...
phases---planning, reasoning, safety checking, inter-agent delegation,
...
Trace Context; (3)~a controlled fault-injection study across 14~fault
types, 6~LLM models, 7~frameworks, and 4~observability conditions
demonstrating that \textsc{AgentTelemetry}
achieves a Fault Detection Rate (FDR) of~1.000 under controlled
fault injection compared to~0.429 for
vanilla OpenTelemetry and~0.000 without telemetry; and (4)~a
pip-installable library (3{,}700+ LOC, 78~tests) with adapters for
seven frameworks, including an end-to-end validated LangChain integration
producing correct spans from real \texttt{AgentExecutor} traces.
A case study on 36 SWE-bench Lite instances reveals that reasoning loops account for 67\% of agent failures---a failure mode invisible to vanilla OTel but precisely characterized by \textsc{AgentTelemetry}'s novel REASONING span kind.
A telemetry-guided intervention that detects and breaks these loops
improves the patch rate by $+11$~pp over a no-intervention control
(3-seed mean, $p{<}0.05$), demonstrating that
the observability is actionable.
Instrumentation overhead is under~5\,ms per span,
negligible relative to LLM API latencies of 500\,ms--10\,s.
\end{abstract}
...
execution.  Five such phases---planning, reasoning, safety checking,
...
\item \textbf{A controlled fault-injection evaluation} across 14~fault
  types, 6~models, 7~frameworks, and 4~observability conditions,
  demonstrating FDR\,=\,1.000 versus 0.429 for vanilla OTel and 0.000
  without telemetry (\S\ref{sec:evaluation}).
...
spans; a safety check and a reasoning chain are both undifferentiated
...
Safety/policy check & (none)               & \textbf{GUARD\_RAIL} & $\star$ \\
...
LOC, 78~tests, MIT license) built on the OpenTelemetry SDK.
...
reasons over results, checks safety constraints, and generates an
...
\centering
\small
\caption{Instrumentation overhead measured across 10{,}000 iterations.}
\label{tab:overhead}
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Operation} & \textbf{Latency} \\
\midrule
Single span creation            & $<$ 5\,ms \\
Nested trace (4 span kinds)     & $<$ 10\,ms \\
Memory per span                 & $<$ 10\,KB \\
Throughput                      & $>$ 1{,}000 spans/sec \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:overhead} shows instrumentation overhead.  LLM API
latencies range from 500\,ms to 10\,s; at $<$5\,ms per span,
\textsc{AgentTelemetry} adds $<$1\% overhead to end-to-end agent
execution.  Memory usage scales linearly with span count at
\texttt{METADATA\_ONLY} level; at \texttt{FULL} level, memory is
dominated by prompt/completion content.
...
The tracing itself adds $<$1\,ms of local processing per span,
...
it on 36~instances from SWE-bench Lite~\cite{swebench} across
...
instance.  Total cost: \$0.85.

\textbf{Results.}
The agent produced 955~spans (REASONING~30.4\%, LLM\_CALL~28.3\%,
RETRIEVAL~19.4\%, MEMORY~7.5\%, TOOL\_CALL~5.3\%, PLANNING~3.8\%,
AGENT~3.8\%, GUARD\_RAIL~1.6\%).  Of 36~instances, 12 (33\%) produced
patches with GUARD\_RAIL verification; 24 (67\%) exhausted the
iteration limit.

\textbf{Fault-type distribution.}
\textsc{AgentTelemetry}'s analysis modules identified two dominant
failure modes in the 24~failed instances:

\begin{itemize}[nosep]
\item \textbf{Reasoning loop (67\% of instances):} The agent repeatedly
...
\item \textbf{Tool failure (31\% of instances):} \texttt{read\_file}
...
failure mode: without it, the 24~reasoning-loop failures would appear
...
\midrule
Recovery (of 24 failed) & 0/24  & 10/24 (42\%) & 12.7$\pm$1.2/24 (53\%) \\
Combined patch rate     & 33\%  & 61\%        & 69$\pm$3\%   \\
\midrule
\textbf{Improvement vs control} & \multicolumn{3}{c}{\textbf{+11 pp} (3-seed mean)} \\
\bottomrule
\end{tabular}
\end{table}
...
0, 0.3, 0.7) and a no-intervention control (10~iterations, same model).
The control recovers 42\% of previously-failed instances through
additional iterations alone.  The telemetry-guided intervention adds
$+11$~pp on top ($53\%\pm4.8\%$ vs $42\%$ recovery; combined patch rate
$69\%\pm3\%$ vs $61\%$), isolating the contribution of the REASONING-based
reasoning-loop detector from the effect of extra iterations.
...
not capture diagnostic quality.  The case studies in RQ4 provide
qualitative evidence of diagnostic value.
...
\emph{Fourth}, the taxonomy was derived by a single researcher; while
the four-stage methodology, saturation check, and MAST cross-validation
(85.7\% convergence) mitigate single-coder bias, inter-rater reliability
from a second coder would further strengthen the qualitative analysis.
\emph{Fifth}, the SWE-bench case study uses 36 of 300 available instances (12\%) with a single model (GPT-4o-mini) and single seed; the closed-loop improvement (+38.9~pp) should be interpreted as a proof-of-concept demonstration rather than a statistically precise estimate. Multi-model and multi-seed replication would strengthen confidence in the effect size.
...
\midrule
\multicolumn{8}{@{}l}{\emph{Safety \& guardrails}} \\
...
These systems enforce safety \emph{policies}; \textsc{AgentTelemetry}
provides \emph{observability} for safety checks via \texttt{GUARD\_RAIL}
spans, enabling operators to monitor whether guardrails fired,
bypassed, or failed---complementary to enforcement.
...
phases---task decomposition, chain-of-thought, safety verification,
