"""OpenAI SDK direct agent using manual AgentTelemetry instrumentation.

Simulates direct use of the OpenAI SDK for function-calling:
1. LLM_CALL spans with llm.provider=openai
2. TOOL_CALL spans for function calls
...
def run_openai_agent(
    task: str = "Summarize the key findings from this dataset",
    ...
    """Run an OpenAI SDK-style function-calling agent with manual instrumentation.

    Simulates the standard OpenAI SDK agentic loop:
    call GPT -> check for function_call -> execute tool -> feed results back

    Args:
        task: The task for the agent to perform.
        mock_client: A MockOpenAIClient or MockAnthropicClient instance.
    ...
    if own_provider:
        provider = configure(service_name="openai-sdk", console=True)
    ...
    iteration = 0
    current_agent = "openai-agent"

    while iteration < max_iterations:
        iteration += 1

        # LLM call span — OpenAI style
        with start_agent_span(f"openai-call-{iteration}", AgentSpanKind.LLM_CALL, tracer=tracer,
                              attributes={
                                  LLM_MODEL: model,
                                  LLM_PROVIDER: "openai",
                              }) as llm_span:
            if mock_client:
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        ...
                        with start_agent_span(f"function-{block.name}", AgentSpanKind.TOOL_CALL, tracer=tracer,
...
run_openai_sdk_agent = run_openai_agent
...
if __name__ == "__main__":
    ...
    result = run_openai_agent(mock_client=client)
    print(f"\nOpenAI SDK agent completed in {result['iterations']} iterations")
